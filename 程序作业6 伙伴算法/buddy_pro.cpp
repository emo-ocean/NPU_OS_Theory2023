...
void reclaim(pair<int, int> range);
...
int main()
{
    ...
    allocate(2);
    allocate(64);
    reclaim(make_pair(64, 127));
    allocate(56);

    // Reclaim memory
    //reclaim(make_pair(0, 31));
    //reclaim(make_pair(40, 47));
...
void reclaim(pair<int, int> range)
{
    // Add the reclaimed range back to the free list
    int n = static_cast<int>(ceil(log(range.second - range.first + 1) / log(2)));
    free_list[n].push_back(range);

    // Merge adjacent free blocks to form a larger free block
    while (n < length - 1 && free_list[n].size() > 1)
    {
        pair<int, int> block1 = free_list[n][free_list[n].size() - 1];
        free_list[n].pop_back();

        pair<int, int> block2 = free_list[n][free_list[n].size() - 1];
        free_list[n].pop_back();

        pair<int, int> merged_block = make_pair(block2.first, block1.second);
        free_list[n + 1].push_back(merged_block);
